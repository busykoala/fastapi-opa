...
class SAMLAuthentication(AuthInterface):
    ...
    async def single_log_out_from_idp(self, request: Request) -> (
            Union[RedirectResponse, Dict]):
    ...
    async def single_log_out(auth: OneLogin_Saml2_Auth) -> RedirectResponse:
        ...
        return RedirectResponse(auth.logout(
            ...
            name_id_format=name_id_format, spnq=name_id_spnq), status_code=303)

    @staticmethod
    async def single_sign_on(
            auth: OneLogin_Saml2_Auth, url: str = None) -> RedirectResponse:
